...
T = TypeVar('T')
'''
las clases genericas las vamos a usar para encapsular operaciones que no son especificas de un tipo
de datos determinados.
En otras palabras, para que el mismo tipo de dato que entre sea el mismo que salga.
...
class CircularDoublyLinkedList(Generic[T]):
    ...
    def append(self, new_value: T) -> None:
        '''
        agrega un nuevo nodo al final de la linked list
        '''
        if self.is_empty():
